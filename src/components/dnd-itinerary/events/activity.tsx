...
export const Activity = memo(function Activity({ event: activity, dayIndex, isSelected }: DndEventProps) {
  const { userId, selectEvent, createEvent, updateEvent, deleteEvents } = useDndItineraryContext();

  const [isOpen, setIsOpen] = useState(false);
  const { setNodeRef, active, over, isDragging, attributes, listeners, transform, transition } = useSortable({
    id: activity.id,
    ...
    >
      {!isSorting && (
        <>
          ...
          <ActivityDetails activity={activity} isOpen={isOpen} setIsOpen={setIsOpen} onClose={handleClose} onDelete={handleDelete} />
        </>
      )}
    </motion.li>
  );
