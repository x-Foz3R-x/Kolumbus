...
export const ActivityOverlay = memo(function EventOverlay({ event, selectCount, hoverShadow }: EventOverlayProps) {
      ...
      >
        ...
        <div className="relative mx-1 mt-0.5 flex h-6 flex-shrink-0 items-center overflow-hidden whitespace-nowrap bg-transparent text-sm text-gray-900">
          <div ref={scrollRef} className="w-full select-none">
            {event.name}
            <ScrollIndicator scrollRef={scrollRef} className={cn(selectCount > 0 && "from-kolumblue-200")} />
          </div>
        </div>
      </div>
    </Badge>
