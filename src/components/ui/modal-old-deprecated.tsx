...
export function ModalOld({ showModal, modalChildren }: ModalProps) {
  return (
    <div
      ...
      className={`fixed inset-0 z-50 flex h-screen w-screen min-w-fit items-center justify-center p-10 duration-300 
        ${showModal ? "scale-100 ease-kolumb-overflow " : "scale-0 ease-kolumb-out"}`}
    >
      <div
        className={`
  ...
  const renderListItems = (header: any) => {
    return list
      ...
      .map((item, index) => (
        <li
          key={`listItem${index}`}
          className="w-32 overflow-hidden text-ellipsis whitespace-nowrap rounded px-2 text-sm text-gray-500 [&:nth-child(odd)]:bg-gray-50"
        >
  ...
  return (
    <div className="flex gap-3 overflow-auto rounded-lg bg-gray-100 px-2 py-3">
      {uniqueHeaders.map((header, index) => (
        <div key={`header${index}`} className="h-full min-w-fit max-w-[8rem] rounded-md bg-white p-2 shadow-kolumblue">
          <h2 className="mb-2 border-b border-gray-200 px-2 pb-1 text-center text-sm font-medium text-gray-600">{header}</h2>
  ...
  return (
    <section className="flex justify-end gap-3 bg-gray-50 px-5 py-3 text-sm font-medium">
      <button
        ...
        className="rounded-lg bg-gray-100 px-5 py-[6px] capitalize text-gray-800 shadow-button duration-200 ease-kolumb-overflow hover:scale-105"
      >
        Cancel
      </button>
...
export function EventsOnExcludedDaysModal(eventsToDelete: Event[], handleExcludedDays: React.MouseEventHandler<HTMLButtonElement>) {
  return (
    <>
      <ModalBody type="exclamation">
        ...
        <ModalMessage>Are you sure you want to proceed and permanently delete the mentioned events?</ModalMessage>
      </ModalBody>
