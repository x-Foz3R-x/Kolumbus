...
export default function TooltipOld({
  triggerRef,
  isOpen,
  setOpen,
  placement,
...
export function TooltipTrigger({ buttonProps, ...tooltip }: TooltipTriggerProps) {
  ...
  return (
    <>
      ...
      <TooltipOld triggerRef={ref} isOpen={isOpen} setOpen={setOpen} {...tooltip} />
    </>
  );
}
...
export function useTooltip(delay: number = 1000) {
  ...
  const handleMouseMove = (event: React.MouseEvent) => {
    ...
    moveTimeoutRef.current = setTimeout(() => setTooltipPosition({ x: event.clientX, y: event.clientY + 18 }), 2000);
  };
