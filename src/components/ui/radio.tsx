      ...
      <ul className="flex items-center gap-1">
        {options.map((option, index) => (
          <li key={`option_${index}`} className="flex items-center">
            ...
            <label
              htmlFor={`${name}-${index}`}
              className={`group relative flex-1 cursor-pointer select-none rounded-full bg-kolumbGray-300 fill-white p-1 capitalize text-kolumbGray-500 shadow-container duration-200 ease-kolumb-flow hover:scale-110 hover:bg-kolumbGray-400 peer-checked:bg-kolumblue-500`}
            >
              {option.element}
              <span className="pointer-events-none absolute bottom-[26px] left-1/2 z-20 -translate-x-1/2 scale-90 rounded bg-kolumbGray-100/50 px-1 py-[2px] text-center text-kolumbGray-500 opacity-0 shadow-container backdrop-blur-[2px] backdrop-saturate-[180%] backdrop-filter duration-150 ease-kolumb-flow group-hover:opacity-100">
                {option?.name}
              </span>
            </label>
          </li>
