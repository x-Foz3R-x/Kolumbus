...
import { FloatingFocusManager, autoUpdate, useDismiss, useFloating, useInteractions, useListNavigation, useRole } from "@floating-ui/react";
import { z } from "zod";
...
type ComboboxProps = {
  root: {
    ...
    activeItemRef: React.MutableRefObject<unknown | null>;
  ...
  input: {
    ...
    className?: string;
    containerProps?: { style?: React.CSSProperties; className?: string } & Omit<AnimationProps, "variants">;
    children?: React.ReactNode;
  } & AnimationProps;
  list: {
...
export const Combobox = {
  Root({ open, setOpen, inputValue, setInputValue, activeItemRef, list, className, children }: ComboboxProps["root"]) {
    const [activeIndex, setActiveIndex] = useState<number | null>(null);
    const listRef = useRef<Array<HTMLLIElement | null>>([]);

    //#region Floating UI
    const data = useFloating<HTMLInputElement>({ whileElementsMounted: autoUpdate, open, onOpenChange: setOpen });
    ...
    const contextValue = useMemo(
      ...
      [open, setOpen, inputValue, setInputValue, activeIndex, setActiveIndex, activeItemRef, list, listRef, interactions, data],
    );
  ...
  Input({ placeholder, onInputChange, onEnterPress, containerProps, className, children, ...animateProps }: ComboboxProps["input"]) {
  ...
  List({ width = { initial: 0, target: 0 }, height = { option: 0, padding: 0, min: 0 }, className, children }: ComboboxProps["list"]) {
    const combobox = useComboboxContext();

    const animateHeight = useMemo(
      () => (combobox.list.length === 0 ? `${height.min}px` : `${combobox.list.length * height.option + height.padding}px`),
            ...
            <FloatingFocusManager context={combobox.context} initialFocus={-1} visuallyHiddenDismiss>
              <motion.ul
                ...
                animate={{
                  ...
                  transition: { ease: EASING.kolumbFlow, duration: 0.4, height: { ease: EASING.anticipate, duration: 0.6 } },
                }}
                exit={{
                  scaleY: 0,
                  width: `${width.initial}px`,
  ...
  Option({ index, className, children }: ComboboxProps["option"]) {
    ...
    const handleClick = () => {
      (combobox.refs.domReference.current?.querySelector("input, textarea") as HTMLElement)?.focus({
        preventScroll: true,
      });
    ...
    return (
      <motion.li
        ref={(node) => (combobox.listRef.current![index] = node)}
        initial="initial"
        animate="animate"
        exit="exit"
        variants={{
          ...TRANSITION.appearInSequence,
          exit: { ...TRANSITION.appearInSequence.exit, scale: 0.5, transition: { ease: EASING.kolumbOut, duration: 0.3 } },
        }}
        custom={{ index }}
        className={cn(
        ...
        {...combobox.getItemProps({ id: useId(), role: "option", "aria-selected": active, onClick: handleClick })}
      >
        <div className={cn("flex items-center gap-2 duration-200 ease-kolumb-overflow", active && "translate-x-1.5")}>{children}</div>
      </motion.li>
    );
  },
};
