...
export const ScrollIndicator = memo(function ScrollIndicator({
  ...
  const StartComponent = memo(function StartComponent() {
    const style = vertical ? { top, insetInline: insetX, height: size, zIndex } : { left, insetBlock: insetY, width: size, zIndex };
    const customStyles = typeof className === "object" ? cn(className.both, className.start) : className;
  ...
  const EndComponent = memo(function EndComponent() {
    const style = vertical ? { bottom, insetInline: insetX, height: size, zIndex } : { right, insetBlock: insetY, width: size, zIndex };
    const customStyles = typeof className === "object" ? cn(className.both, className.end) : className;
