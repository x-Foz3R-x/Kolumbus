...
type ClassName = { progress?: string; progressBar?: string; progressValue?: string; label?: string };
...
export const Progress = memo(function Progress({ outsideLabel, insideLabel, value = 0, max = 1, levels, className }: ProgressProps) {
  ...
  return (
    <div
      className={cn("flex flex-col gap-0.5 text-xs font-medium leading-none tracking-tight", className?.progress, levelClassName?.progress)}
    >
      <OutsideLabel
        label={outsideLabel}
...
const OutsideLabel = memo(function OutsideLabel({ label, percentage, value, max, className }: OutsideLabelProps) {
  if (typeof label === "undefined") return null;

  if (typeof label === "string") return <p className={cn("w-full text-center", className)}>{label}</p>;
...
function generateLabelContent(label: string | LabelType | undefined, percentage: number, value: number, max: number) {
