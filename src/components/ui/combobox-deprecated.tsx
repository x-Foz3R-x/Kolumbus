...
type ComboboxProps = {
  ...
  input: InputProps & { wrapperClassName?: string; children?: React.ReactNode };
...
const Combobox = {
  Root({ isOpen, setOpen, list, onClick, onChange, className, children }: ComboboxProps["root"]) {
    ...
    useCloseTriggers([ref], () => setOpen(false), isOpen);
  ...
  Input({ value, className, wrapperClassName, children, ...props }: ComboboxProps["input"]) {
    const { inputRef, isOpen, listId } = useComboboxContext();
    const childrenRef = useRef<HTMLSpanElement>(null);

    return (
      <div style={{ paddingRight: childrenRef.current?.offsetWidth ?? 0 }} className={wrapperClassName}>
        <Input
          id={useId()}
          ref={inputRef}
          type="text"
        ...
        <span ref={childrenRef} className="absolute inset-y-0 right-0 flex h-full items-center">
          {children}
        </span>
      </div>
    );
