...
export default function Main({ className, children }: Props) {
  return (
    <main
      ...
      className={`relative h-[calc(100vh-3.5rem)] w-[calc(100vw-14rem)] overflow-x-hidden overflow-y-scroll rounded-tl-lg border-l border-t border-kolumbGray-100 bg-cover bg-center bg-no-repeat ${className}`}
    >
      {children}
    </main>
  );
