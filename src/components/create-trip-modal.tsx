...
export const CreateTripModal = memo(function CreateTripModal({ onCreate: handleCreate, buttonProps }: Props) {
    ...
    <>
      ...
      <Modal open={isOpen} setOpen={setOpen} dismissible>
        <ModalBody.iconDesign icon={<Icon.defaultTrip />}>
          ...
          <ModalText>Enter details below to create a new trip and start planning your itinerary.</ModalText>

          <Input label="Trip name" onChange={(e) => (nameRef.current = e.target.value)} variant="insetLabel" />
        </ModalBody.iconDesign>

        <ModalControls>
          <Button onClick={() => setOpen(false)} whileHover={{ scale: 1.05 }} animatePress className="px-5">
            Cancel
          </Button>
          <Button
