...
const InputVariants = cva(
  "peer w-full appearance-none text-gray-800 outline-none outline-0 placeholder:text-gray-400 focus:outline-none disabled:pointer-events-none disabled:bg-gray-100 dark:text-white dark:placeholder:text-gray-600",
  {
    variants: {
      variant: {
...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(inputProps, ref) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    else if (e.key === (KEY.Enter as string)) e.currentTarget.blur();
  };
  ...
  return (
    ...
    >
      ...
      <motion.input
        ...
        className={cn(InputVariants({ variant, size, className: className?.input }), dynamicWidth && "absolute inset-0 h-full text-center")}
        {...props}
      />
      ...
      {!!insetLabel && (
        <label
          ...
          className={cn(
            ...
            value !== undefined && value.toString().length > 0 && "-translate-y-1.5 translate-x-[3px] scale-[.8]",
            className?.label,
          )}
        >
  ...
  return (
    <>
      <OTPInput
        ...
        containerClassName={cn("flex h-fit w-fit select-none items-center gap-2.5 has-[:disabled]:opacity-50", className?.container)}
              ...
              >
                ...
                {slot.hasFakeCaret ? (
                  <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                    <div className="animate-caret-blink h-[18px] w-[1.5px] bg-gray-800" />
                  </div>
                ) : (
                  !slot.char && "•"
                )}
