...
import { motion, MotionStyle, useMotionTemplate, useScroll, useTransform } from "framer-motion";
...
export default function ProgressiveBackgroundColor({
  ...
  const calculateColor = (colorIndex: number) => {
    ...
    const difference = i + 1 >= division.length ? 0 : (colors[i + 1][colorIndex] - colors[i][colorIndex]) * division.length;
