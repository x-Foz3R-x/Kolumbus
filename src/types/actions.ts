...
export enum UT {
  ...
  /**
   * dispatchUserTrips({type: UT.INSERT_TRIP, payload: { trip },});
   */
  INSERT_TRIP = "insert_trip",

  /**
   * dispatchUserTrips({ type: UT.INSERT_EVENT, payload: { selectedTrip: number, dayIndex: number, position: number, event: Event } });
   */
